$(document).ready(function(){
    ...
    //결제 리스트 출력 
    $.ajax({
    	url : 'http://localhost:8080/server/par/payment/passenger/'+p_idx,
    	type: 'GET',
...
//    			output += '<tr><th scope="row">총 운행거리/총 운행시간</th><td>'+data[i].d_distance+' km/ '+(data[i].d_endtime-data[i].d_starttime)+' 분</td></tr>';
    			...
    			output += '</tbody></table></div><a href="'+'http://localhost:8080/passenger/review/passengerWrite.jsp?payidx='+data[i].payidx+'" class="btn btn-primary rvBtn">후기작성</a></div></div></div>';
